...
int eGen_ObtenerIndexLibre(eGen array[], int TAM) {
	...
	//si el array es real, y si el límite es válido.
	if (array != NULL && TAM > 0) {
		//recorro el array.
		for (i = 0; i < TAM; i++) {
			//pregunto si está libre.
			if (array[i].isEmpty == LIBRE) {
				//obtengo la dirección del array disponible y rompo la iteración.
				confirmar = i;
				break;
			}
		}
...
int eGen_BuscarPorID(eGen array[], int TAM, int ID) {
	...
	//si el array existe y el límite es válido.
	if (array != NULL && TAM > 0) {
		//recorro el array.
		for (i = 0; i < TAM; i++) {
			//busco si está ocupado y si coincide con el id que estoy buscando.
			if (array[i].idGen == ID && array[i].isEmpty == OCUPADO) {
				//si se encuentra el id, devuelvo la dirección/index donde está ese id y rompo la iteración.
				confirmar = i;
				break;
			}
		}
